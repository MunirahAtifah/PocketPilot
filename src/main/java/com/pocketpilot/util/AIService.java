...
public class AIService {
    ...
    public static String chatWithAI(String userMessage, String budgetContext) {
        try {
            if (GROQ_API_KEY == null || GROQ_API_KEY.trim().isEmpty()) {
                return "PocketPilot AI is currently offline (API key not configured). Please ask your system administrator to set up the GROQ_API_KEY environment variable.";
            }

            String prompt = "System Context / Instructions:\n"
                    + "You are PocketPilot AI, a financial planning and tracking assistant for students.\n"
                    + "Your purpose is to help students manage their money, understand their budgets, and explain the PocketPilot system.\n"
                    + "Here is how the PocketPilot system works:\n"
                    + "- Budgets: Students can set monthly limits for categories like Food, Utilities, Transport, Education, Entertainment, Healthcare, Shopping, and Other.\n"
                    + "- Expenses: Students log their daily spending. The AI automatically suggests categories based on descriptions.\n"
                    + "- Supervision: This is how parent and counsellor supervision works:\n"
                    + "  1. Parent Access: A parent links their account using a unique 6-character code generated by their child on the Student's 'Supervision' page. Once linked, the parent can view the child's dashboard and tracking progress.\n"
                    + "  2. Counsellor Access: A student counsellor does NOT use a code. Instead, counsellors have a student list in their dashboard where they can see all students and directly click 'Connect' to send a supervision request. The student then receives the request and can approve/decline it on their Supervision page.\n"
                    + "  3. Both parents and counsellors can view budgets/expenses and add comments to help guide the student's financial habits.\n"
                    + "- Reports: Students can export their budget and expense reports as PDFs.\n"
                    + "- Dashboard: Displays statistics like daily average, utilization percentage, top categories chart, and budget vs expense chart.\n\n"
                    + "User Current Budget Context:\n"
                    + budgetContext + "\n\n"
                    + "User Message:\n"
                    + userMessage + "\n\n"
            ...
            System.err.println("[AIService] Chat with AI failed: " + e.getMessage());
            return "Sorry, I am unable to process your request at this time. Error: " + e.getMessage();
        }
    }
}